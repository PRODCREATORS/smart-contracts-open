...
export default async function deploy() {
    ...
    const mnemonic = "poem depth dilemma pattern boat invest federal travel season slogan laugh humble";

    for (let i = 0; i < testNetKeys.length; i++) {
        if(net != testNetKeys[i]){
             continue;
        }
        const provider = new providers.JsonRpcProvider(config.networks[testNetKeys[i] as keyof typeof config.networks].provider);
        const wallet = ethers.Wallet.fromMnemonic(mnemonic).connect(provider);
        chainConfig.providers[`${testNetKeys[i]}Provider`] = provider;

        chainConfig.synthFactories[`${testNetKeys[i]}SynthFactory`] = EntangleSynthFactory__factory.connect(config.networks[testNetKeys[i] as keyof typeof config.networks].factory, wallet)

        // let addrSynth = await chainConfig.synthFactories[`${testNetKeys[i]}SynthFactory`].synths(config.networks[net].chainId, config.networks[net].chef,  config.networks[net].pid);

        // if(addrSynth != ethers.constants.AddressZero) {
        //     continue;
        // }

        let addr = await chainConfig.synthFactories[`${testNetKeys[i]}SynthFactory`].previewSynthAddress(
        ...
        let tx = await  chainConfig.synthFactories[`${testNetKeys[i]}SynthFactory`].createSynth(config.networks[net].chainId, config.networks[net].chef, config.networks[net].pid, config.networks[net].stable, {from: wallet.address});
        console.log(tx);
        await tx.wait();
