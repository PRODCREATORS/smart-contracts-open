...
const formatSynthHashKey = (x: string) => {
  ...
  const chainIdMaxLen = Math.max(...Object.values(_synthInfo).map(e => e.chainId).map(e => e.length));
  const pidMaxLen = Math.max(...Object.values(_synthInfo).map(e => e.pid).map(e => e.length));
