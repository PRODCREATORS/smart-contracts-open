...
export default async function deploy() {
    ...
    for (let i = 0; i < configKeys.length; i++) {
        const provider = new providers.JsonRpcProvider(config.Networks[configKeys[i] as keyof typeof config.Networks].provider);
        chainConfig.providers[`${configKeys[i]}Provider`] = provider;

        chainConfig.synthFactories[`${configKeys[i]}SynthFactory`] = EntangleSynthFactory__factory.connect(config.Contracts[testNetKeys[i] as keyof typeof config.Contracts].factory, provider)
    }
    ...
    switch (net) {
        case "tftm": {
            SynthChef = FantomSynthChef__factory.connect(config.Contracts.tftm.chef, chainConfig.providers.fantomProvider);
            delete chainConfig.synthFactories['fantomSynthFactory'];
            break;
        }

        case "tavax": {
            SynthChef = AvaxSynthChef__factory.connect(config.Contracts.tavax.chef, chainConfig.providers.avaxProvider);
            delete chainConfig.synthFactories['avaxSynthFactory'];
            break;
        }

        case "tbsc": {
            SynthChef = BSCSynthChef__factory.connect(config.Contracts.tbsc.chef, chainConfig.providers.bscProvider);
            delete chainConfig.synthFactories['bscSynthFactory'];
            break;
        }

        case "top": {
            SynthChef = OptimismSynthChef__factory.connect(config.Contracts.top.chef, chainConfig.providers.optimismProvider);
            delete chainConfig.synthFactories['optimismSynthFactory'];
            break;
        }

        case "tarb": {
            SynthChef = ArbitrumSynthChef__factory.connect(config.Contracts.tarb.chef, chainConfig.providers.arbitrumProvider);
            delete chainConfig.synthFactories['arbitrumSynthFactory'];
            break;
        }

        case "tmat": {
            SynthChef = PolygonSynthChef__factory.connect(config.Contracts.tmat.chef, chainConfig.providers.polygonProvider);
