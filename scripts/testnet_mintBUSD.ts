...
const getSynthInfo = () => {
  ...
  const files = readdirSync(deploys).filter((e) =>
    /^t[a-z]+_addresses/g.test(e)
  );
...
async function mintAndTransfer(net: string, contract: BEP20Token, amount: BigNumberish, to: string, owner: string) {
  switch (net) {
    case "tftm": {
...
async function main() {
  ...
  for (const [name, net] of Object.entries(netz)) {
    ...
    const selfBUSD = await ethers.getContractAt(
      "BEP20Token",
      net.opToken,
      signer
    );
    const bal = await selfBUSD.balanceOf(self.address);
    const toMint = ethers.BigNumber.from("1337")
