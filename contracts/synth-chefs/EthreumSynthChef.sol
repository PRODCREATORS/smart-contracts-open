...
contract EthreumSynthChef is BaseSynthChef {
        ...
        returns (TokenAmount[] memory tokenAmounts)
    {
        ...
        tokenAmounts[0] = TokenAmount({token: pool.token0, amount: pool.curvePool.balances(0)*_amount / IERC20(pool.lp).totalSupply()});
        tokenAmounts[1] = TokenAmount({token: pool.token1, amount: pool.curvePool.balances(1)*_amount / IERC20(pool.lp).totalSupply()});
        tokenAmounts[2] = TokenAmount({token: pool.token2, amount: pool.curvePool.balances(2)*_amount / IERC20(pool.lp).totalSupply()});
        ...
        returns (TokenAmount[] memory tokenAmounts)
    {
        ...
        uint256 amount2 = pool.curvePool.calc_withdraw_one_coin(
            amountLP,
            uint256(2)
        );
    ...
    function addPool(
        ...
        poolsArray.push(
            Pool(_lp, _convexID, _token0, _token1, _token2, _curvePool, _convexreward)
        );
    }
}
