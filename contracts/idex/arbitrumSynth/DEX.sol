...
contract DEX is AccessControlEnumerable {
        ...
        isActive(_pid)
    {
        ...
        if (synthStruct.crosschain) {
            require(
                IERC20(synthStruct.synth).balanceOf(address(this)) >=
                    amountSynth,
                "Not enough synth to buy"
            );
            IERC20(opToken).transferFrom(msg.sender, address(this), _amount);
            IERC20(synthStruct.synth).transfer(msg.sender, amountSynth);
        } else {
            IERC20(opToken).transferFrom(msg.sender, address(this), _amount);
            ISynthChef(chef).deposit(
                farmPid,
                ETHSynthV1(synthStruct.synth).pid(),
                _amount,
                opToken
            );
            ETHSynthFactoryV1(factory).mint(
                ETHSynthV1(synthStruct.synth).pid(),
                amountSynth,
                msg.sender
            );
        ...
        isActive(_pid)
    {
        ...
        if (synthStruct.crosschain) {
            require(
                IERC20(opToken).balanceOf(address(this)) >= amountOpToken,
                "Not enough opToken to sell"
            );
            ...
            uint256 amountLp = ISynthChef(chef).convertStableToLp(
                farmPid,
                amountOpToken
            );
            ETHSynthFactoryV1(factory).burn(
                ETHSynthV1(synthStruct.synth).pid(),
                msg.sender,
                _amount
            );
            ISynthChef(chef).withdraw(
                farmPid,
                ETHSynthV1(synthStruct.synth).pid(),
                amountLp,
                opToken,
     ...
     * May emit a {RoleGranted} event.
     */
    function addBuyer(address _buyer) external onlyRole(ADMIN) {
        require(!hasRole(BUYER, _buyer), "already buyer");
        grantRole(BUYER, _buyer);
    }

    /**
     * @dev Revokes `BUYER` role from `_buyer`.
     *
     ...
     */
    function removeBuyer(address _buyer) external onlyRole(ADMIN) {
        revokeRole(BUYER, _buyer);
    }

    /**
     * @dev Returns addresses that control `BUYER` role
     */
    function buyers() external view returns (address[] memory) {
        uint256 _buyersLength = getRoleMemberCount(BUYER);
        require(_buyersLength > 0, "No buyers");
        address[] memory _buyers = new address[](_buyersLength);
        for (uint256 i = 0; i < _buyersLength; i++) {
            _buyers[i] = getRoleMember(BUYER, i);
        }
        return _buyers;
    }
